          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-2xl mx-auto">
            {kepalaDesa && (
              <div className="text-center">
                <Image src={kepalaDesa.photo_url ? `${apiUrl}${kepalaDesa.photo_url}` : '/assets/img/placeholder.png'} alt={kepalaDesa.name} width={128} height={128} className="w-32 h-32 rounded-full mx-auto mb-4 object-cover shadow-lg border-4 border-white" />
            ...
            {sekretarisDesa && (
              <div className="text-center">
                <Image src={sekretarisDesa.photo_url ? `${apiUrl}${sekretarisDesa.photo_url}` : '/assets/img/placeholder.png'} alt={sekretarisDesa.name} width={128} height={128} className="w-32 h-32 rounded-full mx-auto mb-4 object-cover shadow-lg border-4 border-white" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {latestNews.map((item) => (
            <Link href={`/berita/${item.slug}`} key={item.id} className="block bg-white rounded-lg shadow-md overflow-hidden group">
              <Image src={item.featured_image_url ? `${apiUrl}${item.featured_image_url}` : '/assets/img/placeholder.png'} alt={item.title} width={400} height={250} className="w-full h-48 object-cover group-hover:opacity-80 transition" />
              <div className="p-6">
